...
- `account_id` (String) The git account to use for private repositories. When set, `domain` and `https_enabled` are derived automatically from the linked account. Omit or set to empty string to disconnect.
...
- `domain` (String) The git provider domain without protocol prefix (e.g. `github.com`). Cannot be set together with `account_id`.
- `https_enabled` (Boolean) Whether to use HTTPS (true) or HTTP (false) for cloning. Defaults to `true`. Cannot be set together with `account_id`.
