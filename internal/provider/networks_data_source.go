...
func (d *NetworksDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for _, serverID := range serverIDs {
		...
		if err != nil {
			resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to list docker networks for server %s, got error: %s", serverID, err))
			return
		}
		for _, n := range networks {
			var item NetworkDataSourceModel
			...
			allNetworks = append(allNetworks, item)
		}
	}
